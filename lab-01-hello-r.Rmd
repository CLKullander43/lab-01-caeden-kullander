...
Three are 1846 rows and 3 variables named *dataset*, *x*, and *y*.
...
star_data %>%
  summarize(r = cor(x, y))
```
The correlation coefficient for star is roughly -0.063

Correlation between star and dino:
```{r}
-0.0629611	/ -0.06447185
```
...
Correlation between circle and dino:
```{r}
-0.06834336	/-0.06447185
```
...
```{r all-viz, fig.fullwidth=TRUE, fig.height=6, fig.width=6}
